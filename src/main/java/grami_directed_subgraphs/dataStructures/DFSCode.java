...
public class DFSCode<NodeType, EdgeType> extends
	...
	protected ArrayList<Integer> sortedFreqLabels;
	...
	protected  GSpanEdge<NodeType, EdgeType> first, last;

	protected int psize;

	protected int threadIdx;
	
	protected HPListGraph<NodeType, EdgeType> me;
	
	protected IntFrequency finalFrequency=null;
	
	protected Graph singleGraph;
	
	protected Variable[] currentVariables=null;
	protected Variable[] previousStepVariables=null;
	
	protected HashMap<Integer, HashSet<Integer>> nonCandidates=null;
	
	public HashMap<Integer, HashSet<Integer>> getNonCandidates() {
		return nonCandidates;
	}

	transient protected ArrayList<GSpanEdge<NodeType, EdgeType>> parents;
	...
	public DFSCode(DFSCode code){
		this.sortedFreqLabels=code.getSortedFreqLabels();
		this.singleGraph=code.getSingleGraph();
		this.nonCandidates=code.nonCandidates;
		this.currentVariables = code.currentVariables;
		this.finalFrequency = code.finalFrequency;
		this.first = code.first;
		this.last = code.last;
		this.me = code.me;
		this.parents = code.parents;
		this.previousStepVariables = code.previousStepVariables;
		this.psize = code.psize;
	...
	public SearchLatticeNode<NodeType, EdgeType> extend(
		...
		//System.out.println("Ext grag::" + ext.toString());
	...
	protected int getNumOfMaxDistinctLabel()
	{
	...
	protected boolean areAllLabelsDistinct()
	{
	...
	protected boolean areAllLabelsTheSame()
	{
...
//			}
	 ...
	 * nodeA/gNodeA
	 */
	protected final MinExtension<NodeType, EdgeType> getExtensions(
	 ...
	 */
	protected boolean isCan(final MinExtensionSet<NodeType, EdgeType> set,
	 ...
	 * @return <code>false</code>, the part of the serach determines this
	 */
	protected boolean isCan2(final GSpanEdge<NodeType, EdgeType> ackEdge,
	...
	protected int[] getIntArray(final int length, final int def) {
	...
	protected int[] getIntArray(final int length) {
...
//		}
	...
	public String DFSCodeString(){
		GSpanEdge<NodeType, EdgeType> spanEdge = this.first;
		String dfsCodes = spanEdge.toString();
		while (spanEdge.next != null){
			spanEdge = spanEdge.next;
			dfsCodes = dfsCodes.concat(spanEdge.toString());
		}
		return dfsCodes;
	}

	public List<GSpanEdge<NodeType, EdgeType>> getGSpanOrder(){
		List<GSpanEdge<NodeType, EdgeType>> dfsCodes = new LinkedList<>();
		GSpanEdge<NodeType, EdgeType> spanEdge = this.first;
		dfsCodes.add(spanEdge);
		while (spanEdge.next != null){
			spanEdge = spanEdge.next;
			dfsCodes.add(spanEdge);
		}
	...
	public String DFSEdgesFromVariable(String a){
		GSpanEdge<NodeType, EdgeType> spanEdge = this.first;
		String dfsCodes = null;
		if(spanEdge.getNodeA() == Integer.valueOf(a) || spanEdge.getNodeB() == Integer.valueOf(a)){
			dfsCodes = spanEdge.toString();
		}
		while (spanEdge.next != null){
			spanEdge = spanEdge.next;
			if(spanEdge.getNodeA() == Integer.valueOf(a) || spanEdge.getNodeB() == Integer.valueOf(a)){
